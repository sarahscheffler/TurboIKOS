...
def compute_output(circuit, epsilon_1, epsilon_2, wire, n_gate, n_parties, n_epsilons):
    ...
    alpha_broadcast = [[None for x in range(len(alpha_shares_mulgate))] for x in range (n_epsilons)] #alpha_broadcast[#mul_gate][#epsilon]
    for i in range(len(alpha_shares_mulgate)):
        for e in range(n_epsilons):
            alpha_broadcast[i][e] = sum(alpha_shares_mulgate[i][e])
...
def compute_zeta_share(circuit, wire, alpha, epsilon_1, epsilon_2, n_parties, n_epsilons):
        ...
        for e in range(n_epsilons):
            ...
            for j in range(len(circuit)):
                if circuit[j].operation == 'AND' or  circuit[j].operation == 'MUL':
                    ...
                    y = circuit[j].y
                    z = circuit[j].z
                    sum_alpha = sum(alpha[n][e])
                    #epsilon_1[e][n], wire.e(y), sum_alpha, wire.lambda_val(x)[i], epsilon_1[e][n], wire.e(x), wire.lambda_val(y)[i], epsilon_1[e][n], wire.lambda_val(z)[i], epsilon_2[e][n], wire.lam_hat(z)[str(n)][i] 
                    zeta += (epsilon_1[e][n] * wire.e(y) - sum_alpha)* wire.lambda_val(x)[i] + \
    ...
    def compute_A_share(alpha_broadcast, alpha_shares_mulgate, rand, n_parties, n_epsilons):
        A = [[None for x in range(n_parties)] for x in range(n_epsilons)]
        for j in range(n_parties):
            for e in range(n_epsilons):
                cum = 0
                for m in range(len(alpha_broadcast)):
                    cum += (rand[m][e]*alpha_shares_mulgate[m][e][j])
                    if j == 0:
                        cum -= (rand[m][e]*alpha_broadcast[m][e])
                A[j][e] = cum
        return A
