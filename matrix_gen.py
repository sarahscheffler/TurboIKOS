...
def gen_circuit(n, m, f, A):
    ...
    output_w = num_wires - num_outputs 
    input_1 = num_inputs
    input_2 = num_inputs + 1
    for i in range(n):
        for j in range(m):
            f.write("\n2 1 %d %d %d SCA" % (input_w, A[i][j].value, internal_w))
            input_w += 1
            internal_w += 1
        ...
        for i in range(m-1):
            if i == m-2:
                f.write("\n2 1 %d %d %d XOR" % (input_1, input_2, output_w))
                output_w += 1
                input_1 += 1
                input_2 = input_1 + 1
            else:
                f.write("\n2 1 %d %d %d XOR" % (input_1, input_2, internal_w))
                input_1 = internal_w
                input_2 += 1
                internal_w += 1
    
    for i in range(m):
        f.write("\n2 1 %d %d %d AND" % (i, i, internal_w))
        internal_w += 1
        f.write("\n2 1 %d %d %d SCA" % (i, -1, internal_w))
        f.write("\n2 1 %d %d %d XOR" % (internal_w-1, internal_w, output_w))
        internal_w += 1
        output_w += 1
...
if __name__ == "__main__":
    ...
    with open("circuits/circuit_%dx%d.txt" % (n,m), 'w') as f:
        gen_circuit(n, m, f, A)
    with open("circuits/input_%dx%d.txt" % (n,m), 'w') as f:
        for i in s:
            f.write("%d " % (i[0].value))
        f.write('\n')
        for i in t:
