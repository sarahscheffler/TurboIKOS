...
def check_commitments(parties, cm_views, cm_round1, cm_round3, cm_round5, dict_rebuilt): 
    #check views 
    ...
    assert(rb_round1 == cm_round1), "Committed round 1 broadcast does not match opened round 1 broadcast"
    #check round 3
...
def recompute(circuit, c_info, parties, comitted_views, open_views, open_round1, open_round3, commit_round3):
    ...
    alpha = [[None for x in range(len(parties))] for x in range(n_mult)] #alpha[mult gate][party]
    ...
    for i in range(len(parties)):
        ...
        for j in range(n_gate):
            ...
            if c.operation == 'MUL' or c.operation == 'AND':
                ...
                alpha[num_mult][i] = epsilon1[num_mult]*y_lam + (epsilon2[num_mult] * y_lamh) #alpha[nummult][party]
    ...
    bigA = [None for x in range(parties)]
    for p in range(len(parties)): 
        sum_smalla = 0 
        for m in range(n_mult):
            sum_smalla += (gammas[p] * alpha[m][p])
            if parties[p] == 0:
                sum_smalla -= (gammas[p]*b_alpha[m])
        bigA[p] = sum_smalla
...
"""
def check_recompute(c_info, parties, dict_broadcast, recomputed_alpha, recompute_output_shares, recomputed_zeta):
    ...
    for i in range(len(parties)): 
        ...
        assert (prover_alpha[current_party].value == recomputed_alpha[i].value), "Verifier's recomputed alphas does not match prover's alphas."
...
def verifier(circuit, c_info, parties, cm_views, cm_round1, cm_round3, cm_round5, open_views, dict_rval, dict_broadcast):
    #check commitments
    rebuild = rebuild_commitments(circuit, c_info, parties, open_views, dict_rval, dict_broadcast)
    check_commitment = check_commitments(parties, cm_views, cm_round1, cm_round3, cm_round5, rebuild)

    #verifier check zeta 
    check_zeta(dict_broadcast['round3']['zeta'])

    #verifier check bigalpha
    check_bigalpha(dict_broadcast['round5'])

    #verifier recompute 
    v_recompute = recompute(circuit, c_info, parties, cm_views, open_views, dict_broadcast['round1'], dict_broadcast['round3'], cm_round3)
