    ...
    def test_full(self):
        ...
        for test in range(1, len(sys.argv)):
            with self.subTest(test = test):
                Circuit = circuit.parse_test(gate, n_parties, test)
                n_wires = Circuit[4]
                n_gate = Circuit[3]
                l_input = Circuit[1]
                n_input = Circuit[6]
                ...
                for i in range(n_gate):
                    ...
                    if g.operation == 'AND' or g.operation == 'MUL':
                        for j in range(n_parties):
                            self.assertEqual(alpha[m][j], epsilon_1[m]*w.lambda_val(g.y)[j] + epsilon_2[m]*w.lam_hat(g.y)[j])
                        m = m + 1
                ...
                for j in range(n_gate):
                    ...
                    if g.operation == 'XOR' or g.operation =='ADD': 
                        #Check v 
                        self.assertEqual(sum(w.v(g.z)) , (sum(w.v(g.x)) + sum(w.v(g.y))))
                        #Check lambda assignment
                        for i in range(n_parties):
                            self.assertEqual(w.lambda_val(Circuit[j].x)[i] + w.lambda_val(Circuit[j].y)[i] , w.lambda_val(Circuit[j].z)[i])
                    #MUL gate
                    if g.operation == 'AND' or g.operation == 'MUL':
                        #Check e hat assignment
                        self.assertEqual(w.e_hat(g.z) , sum(w.lambda_val(g.x)) * sum(w.lam_hat(g.y)) + sum(w.lam_hat(g.z)))
                        #Chck v value
                        self.assertEqual(sum(w.v(g.z)) , (sum(w.v(g.x)) * sum(w.v(g.y)))) 
                #Check e assignment
                for i in range(n_wires):
                    self.assertEqual(w.e(i) , sum(w.lambda_val(i)) + sum(w.v(i)))

                zeta = circuit.compute_zeta_share(Circuit, w, alpha, epsilon_1, epsilon_2, n_parties)
                #Check zeta
                self.assertEqual(sum(zeta).value , 0)
                #Commit to broadcast
                ...
                for i in range(n_input):
                    self.assertEqual(w.e(i) , broadcast['e inputs'][i])
                    for j in range(n_parties):
                        self.assertEqual(full_views[j]['input'][i] , w.v(i)[j])
                        self.assertEqual(full_views[j]['input lambda'][i] , w.lambda_val(i)[j])
                ...
                for i in range(n_gate):
                    ...
                    if c.operation == 'MUL' or c.operation == 'AND':
                        for j in range(n_parties):
                            self.assertEqual(full_views[j]['lambda z'][m] , w.lambda_val(c.z)[j])
                            self.assertEqual(full_views[j]['lambda y hat'][m] , w.lam_hat(c.y)[j])
                            self.assertEqual(full_views[j]['lambda z hat'][m] , w.lam_hat(c.z)[j])
            
                        self.assertEqual(broadcast['e z'][m] , w.e(c.z))
                        self.assertEqual(broadcast['e z hat'][m] , w.e_hat(c.z))
                        m += 1

                self.assertEqual(w.v(Circuit[-1].z) , broadcast['output shares']) 
