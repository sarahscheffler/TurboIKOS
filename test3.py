...
def test():
    ...
    views_size = rep*sum([sum([sum([VALUE_BYTES for v in views[i][k]]) for k.value in views[i]]) for i in range(n_corrupt)])
    views_size_PR = rep*(sum([sum([VALUE_BYTES for v in views[i]["input"]]) for i in range(n_corrupt)]) + SEED_BYTES*t)
