    ...
    def test_seed(self):
        for test in range(1, len(sys.argv)):
            with self.subTest(test = test):
                ...
                n_parties = 3    
                wire_data = prover.wire_data(n_wires)
                ...
                verifier.run_verifier(c_info, v_circuit[0], prover_results)
        ...
        for test in range(1, len(sys.argv)):
            with self.subTest(test = test):
                ...
                for repetition in range(rep): 
                    ...
                    prover_results = prover.run_prover(c_info, Circuit, w, n_parties, inputs, party_seeds)

                    #Start time for verifier
                    start_time = time.process_time()

                    #verifier test
                    verifier.run_verifier(c_info, Circuit, prover_results)
